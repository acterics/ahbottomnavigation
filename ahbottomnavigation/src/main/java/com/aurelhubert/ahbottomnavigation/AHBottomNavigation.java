	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (itemsEnabledStates[i]) {
				...
				icon.setImageDrawable(AHHelper.getTintDrawable(
						items.get(i).getDrawable(context),
						current ? itemActiveColor : itemInactiveColor,
						forceTint,
						item.hasTint())
				);
	...
	private void createSmallItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (itemsEnabledStates[i]) {
				icon.setImageDrawable(AHHelper.getTintDrawable(
						items.get(i).getDrawable(context),
						currentItem == i ? itemActiveColor : itemInactiveColor,
						forceTint,
						item.hasTint())
				);
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				AHHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
						itemInactiveColor, itemActiveColor, forceTint, items.get(itemIndex).hasTint());
				...
				AHHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
						itemActiveColor, itemInactiveColor, forceTint, items.get(currentItem).hasTint());
			}
		}
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (updateStyle) {
				...
				if (notificationBackgroundDrawable != null) {
					...
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
						notification.setBackground(AHHelper.getTintDrawable(defaultDrawable,
								currentBackgroundColor, forceTint));
					} else {
						notification.setBackgroundDrawable(AHHelper.getTintDrawable(defaultDrawable,
								currentBackgroundColor, forceTint));
					}
				}
			}
